...
interface LevelDialogProps {
    ...
    levelName: string;
    levelWeight: string;
    onEditLevel: () => Promise<void>;
}

export default function EditLevelDialog(
    { levelId, levelName, levelWeight, onEditLevel }: LevelDialogProps) {
    ...
    const editLevel = async (): Promise<null> => {
        try {
            const { data, error } = await supabase
                .from('year')
                .update({ name: newLevelName, weight: newLevelWeight })
                .eq('id', levelId);
                            ...
                            <div className="inline-block align-bottom bg-base-100 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                ...
                                <div className="px-6 py-4">
                                    <label className="input input-bordered flex items-center gap-2">
                                        Name
                                        <input type="text" value={newLevelName} onChange={(e) => setLevelName(e.target.value)} className="grow" placeholder="Assignment Name" />
                                    </label>
